...
if __name__ == "__main__":
    total = 0
    hits = 0
    with open('pob.tsv', 'w') as fout, \
            open('pob.json', 'w') as jsonout, \
            open('pob_gold.tsv', 'w') as goldf, \
            open('20130403-place_of_birth.json', 'r') as fin:
        ...
        for i, line in tqdm.tqdm(lines_i):
            ...
            dat = {
                ...
                'overall': 1 if ((sum(judgments) / len(judgments)) > 0.5) else -1
            }
            fout.write('{}\t{}\n'.format(i, dat['text']))
